...
function sourceLine(row) {
  if (row.kind === "invoice_payment" && row.decision === "approve") {
    return "Stripe · payment received & reconciled";
  }
...
function isHero(row) {
  // The single pitch frame: the agent refusing an OFF-GOAL self-spend.
  return (
    row.kind === "self_spend" &&
    row.decision === "block" &&
    !ALLOWED_SELF.includes(row.category)
  );
}
...
function renderMeters(state) {
  el.earnings.textContent = money(state.earnings, "GBP");
  el.spend.textContent = money(state.spend, "GBP");
  el.net.textContent = money(state.net, "GBP");
  el.catch.textContent = pct(state.catch_rate);
  el.catchBar.style.width = pct(state.catch_rate);
}
...
function spotlightEvaluating(row) {
  ...
  el.spAmount.textContent = row.amount != null ? money(row.amount, row.currency) : "";
    ...
    <td>
      <div class="cell-kind">
        ...
        <span class="ktext">
          ...
          <span class="kkind">${escapeHtml(KIND_LABEL[row.kind] || row.kind)}${row.kind === "self_spend" ? '<span class="self-tag"> · SELF</span>' : ""}</span>
        </span>
      </div>
    </td>
...
function renderApproval(state) {
  ...
  el.cardRisk.textContent = pending.risk != null ? `risk ${pending.risk}` : "";
}
...
function showIdle() {
  ...
  renderMeters({ earnings: 0, spend: 0, net: 0, catch_rate: 0 });
  renderCounters([]);
...
async function runDemo() {
  ...
  renderMeters({ earnings, spend, net: 0, catch_rate: catchRate });
  ...
  for (let i = 0; i < ordered.length; i++) {
    ...
    renderMeters({ earnings, spend, net: earnings - spend, catch_rate: catchRate });
    if (earned) bumpFlash(el.earnings.closest(".stat"));
    ...
    spotlightVerdict(row, isHero(row) ? "Self-spend refused" : "Decided");

    // 3) Drop into the decision log (newest at top).
  ...
  el.endStat.textContent = `${full.timeline.length} events · ${t.approve} auto-approved · ${t.escalate} escalated · ${t.block} blocked · net ${money(full.net)}`;
  el.card.classList.add("hidden");
  el.endCard.classList.remove("hidden");

  demoRunning = false;
  el.runDemo.disabled = false;
  setStatusState("complete", `Complete · ${full.timeline.length} decisions`);
}
